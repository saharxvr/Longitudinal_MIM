...
if __name__ == "__main__":
    ...
    print(f"[bulk-v2] device={device}, pair_dirs={len(pair_dirs)}")
    ...
    for idx, pair_dir in enumerate(pair_dirs, start=1):
        try:
            ...
            if idx % 10 == 0 or idx == len(pair_dirs):
                print(f"[bulk-v2] done {idx}/{len(pair_dirs)} pairs | segs={seg_written}")

        except Exception as e:
            failures.append((str(pair_dir), str(e)))
            print(f"[bulk-v2][error] {pair_dir}: {e}")
            traceback.print_exc()
    ...
    print(f"[bulk-v2] complete. summary={summary}")
    if failures:
        sys.exit(1)
