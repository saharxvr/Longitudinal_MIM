...
def resolve_annotation_path(ann_base, physician, pair_num):
    ...
    if len(candidates) == 1:
        return candidates[0]
    # Prefer files in 'new' directories (e.g. Benny has new/ and old/ subfolders)
...
def main(model_outputs_base_path, annotations_base_path, pairs_roots, out_path, num_pairs=60, bias=0):
    ...
    for i in range(bias, num_pairs + bias):
        ...
        try:
            ...
            continue
        current_path = f'{pair_path}/{sorted([p for p in os.listdir(pair_path) if p.endswith(".nii.gz") and "_seg" not in p])[1]}'
    ...
    plot_curves([s[0] / s[1] for s in sensitivities_pos], [s[0] / s[1] for s in sensitivities_neg], '$M_{ICU}$', f'{out_path}/sensitivity_consensus_levels.png')

    plot_curves([s[0] / s[1] for s in sensitivities_pos_avi], [s[0] / s[1] for s in sensitivities_neg_avi], 'A', f'{out_path}/sensitivity_consensus_levels_avi.png')
    plot_curves([s[0] / s[1] for s in sensitivities_pos_benny], [s[0] / s[1] for s in sensitivities_neg_benny], 'B', f'{out_path}/sensitivity_consensus_levels_benny.png')
    plot_curves([s[0] / s[1] for s in sensitivities_pos_sigal], [s[0] / s[1] for s in sensitivities_neg_sigal], 'C', f'{out_path}/sensitivity_consensus_levels_sigal.png')
    plot_curves([s[0] / s[1] for s in sensitivities_pos_smadar], [s[0] / s[1] for s in sensitivities_neg_smadar], 'D', f'{out_path}/sensitivity_consensus_levels_smadar.png')
    plot_curves([s[0] / s[1] for s in sensitivities_pos_nitzan], [s[0] / s[1] for s in sensitivities_neg_nitzan], 'E', f'{out_path}/sensitivity_consensus_levels_nitzan.png')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Observer Variability Analysis')
    parser.add_argument('--model-preds-dir', required=True, help='Path to model predictions directory')
    parser.add_argument('--annotations-dir', required=True, help='Path to annotations base directory')
    parser.add_argument('--pairs-roots', nargs='+', required=True, help='Pair root directories (Pairs1 .. Pairs8)')
    ...
    main(args.model_preds_dir, args.annotations_dir, args.pairs_roots, args.out_dir, args.num_pairs, args.bias)
