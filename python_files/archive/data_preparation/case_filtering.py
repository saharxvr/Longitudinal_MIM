...
def measure_cases_resolution():
    import SimpleITK as sitk

    case_dirs = ['/cs/labs/josko/itamar_sab/LongitudinalCXRAnalysis/CT_scans/LUNA_scans',
...
def crop_devices():
    ...
    for seg_p, scan_p in segs_and_scans_ps:
        ...
        scan_nif = nib.load(scan_p)
        aff = seg_nif.affine

        seg = seg_nif.get_fdata()
        scan = scan_nif.get_fdata()

        seg_coords = seg.nonzero()
        min_1, max_1 = np.amin(seg_coords[0]), np.amax(seg_coords[0])
        min_2, max_2 = np.amin(seg_coords[1]), np.amax(seg_coords[1])
        min_3, max_3 = np.amin(seg_coords[2]), np.amax(seg_coords[2])

        scan = scan * seg
        scan = scan[min_1: max_1, min_2: max_2, min_3: max_3]

        dev_name = seg_p.split('\\')[-1].split('_')[0]

        device_nif = nib.Nifti1Image(scan, aff)
        nib.save(device_nif, fr'C:\Users\sharp\PycharmProjects\LongitudinalCXRAnalysis\CTs_with_devices\{dev_name}.nii.gz')


if __name__ == '__main__':
    measure_cases_resolution()
    exit()
