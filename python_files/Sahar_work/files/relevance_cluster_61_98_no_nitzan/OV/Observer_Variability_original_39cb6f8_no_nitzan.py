...
def resolve_model_output_path(base_path, pair_num):
    # Support both historical folder naming variants: pairN and PairN.
    candidates = [f'{base_path}/pair{pair_num}/output.nii.gz', f'{base_path}/Pair{pair_num}/output.nii.gz']
...
def main():
    ...
    #     {'Avi': [s[0] / s[1] for s in sensitivities_pos_avi], 'Benny': [s[0] / s[1] for s in sensitivities_pos_benny], 'Sigal': [s[0] / s[1] for s in sensitivities_pos_sigal], 'Smadar': [s[0] / s[1] for s in sensitivities_pos_smadar]},
    #     {'Avi': [s[0] / s[1] for s in sensitivities_neg_avi], 'Benny': [s[0] / s[1] for s in sensitivities_neg_benny], 'Sigal': [s[0] / s[1] for s in sensitivities_neg_sigal], 'Smadar': [s[0] / s[1] for s in sensitivities_neg_smadar]},
    #     num_humans - 1,
