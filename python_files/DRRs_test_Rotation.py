...
def tensor_binary_closing(t: torch.Tensor, r: int) -> torch.Tensor:
    # Ensure binary (convert >0 to 1 if not already boolean)
    c_arr = t.detach().cpu().numpy()

    # Create circular structuring element
    structure = disk(r)

    # Perform closing
    closed = binary_closing(c_arr, structure=structure)

    # Convert back to original tensor type
    return torch.as_tensor(closed, dtype=t.dtype, device=t.device)
...
def remove_size_outliers_ccs(im, min_count=16, max_count=122500, with_closing=True):
    # Part 1
    ...
    unique_vals = unique_vals[torch.logical_and(unique_counts > min_count, unique_counts < max_count)]
    ccs = ccs[None, None, ...]
    ccs_indic = torch.isin(ccs, unique_vals)
    new_im = im * ccs_indic
    if with_closing:
        new_im = tensor_binary_closing(new_im.squeeze(), r=2)[None, None, ...]

    # Part 2
    ccs, ccs_num = label(new_im.cpu().squeeze(), struct)
    ccs = torch.from_numpy(ccs).to(DEVICE)
    return new_im, ccs, ccs_num
...
def create_violin_plot(pos_arrs, neg_arrs, title, y_label, save_path):
    ...
    plt.title(title)
    plt.xticks(ticks=ticks, labels=['None', 'Low', 'Normal', 'High'], rotation=12, fontsize=10)
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        entity_name = 'fluid_overload'
        plots_folder = f'/cs/labs/josko/itamar_sab/LongitudinalCXRAnalysis/CT_scans/{entity_name}_test/results'
        DRR_pairs_folder = f'/cs/labs/josko/itamar_sab/LongitudinalCXRAnalysis/CT_scans/{entity_name}_test'
        ...
        # rot_angles_lst = [(0, 0, 0), (8, 8, 8), (15, 15, 15), (25, 25, 25)]
        rot_angles_lst = [(15, 15, 15)]
        ...
        for idx, rot_angles in enumerate(rot_angles_lst):
            ...
            c_plots_folder = f'{plots_folder}/{entity_name}_rotation_invariance_angles_{rot_angles[0]}_{rot_angles[1]}_{rot_angles[2]}'
            ...
            for i in tqdm(range(0, ds_len, 1)):
                ...
                if torch.any(torch.isnan(bl)) or torch.any(torch.isnan(fu)):
                    continue
                ...
                if sharpen:
                    ...
                    # bls_min = bl.amin(dim=(1, 2, 3), keepdim=True)
                    # bls_max = bl.amax(dim=(1, 2, 3), keepdim=True)
                    # bl = (bl - bls_min) / (bls_max - bls_min)
                    #
                    # fus_min = fu.amin(dim=(1, 2, 3), keepdim=True)
                    # fus_max = fu.amax(dim=(1, 2, 3), keepdim=True)
                    # fu = (fu - fus_min) / (fus_max - fus_min)

                    bl = (bl - torch.min(bl)) / (torch.max(bl) - torch.min(bl))
                    fu = (fu - torch.min(fu)) / (torch.max(fu) - torch.min(fu))

                    bl = kornia.enhance.equalize_clahe(bl, clip_limit=1.25, grid_size=(8, 8))
                    fu = kornia.enhance.equalize_clahe(fu, clip_limit=1.25, grid_size=(8, 8))

                    bl = adjust_sharpness(bl, sharpness_factor=4.)
                    fu = adjust_sharpness(fu, sharpness_factor=4.)

                    bl = np.clip(bl, a_min=0., a_max=1.)
                    fu = np.clip(fu, a_min=0., a_max=1.)
                ...
                outs_pos_th, _, __ = remove_size_outliers_ccs(outs_pos_th, min_count=40, max_count=max_count, with_closing=False)
                outs_neg_th, _, __ = remove_size_outliers_ccs(outs_neg_th, min_count=40, max_count=max_count, with_closing=False)
                ...
                gts_pos_th, gts_pos_ccs, gts_pos_ccs_num = remove_size_outliers_ccs(gts_pos_th, min_count=180, max_count=max_count, with_closing=True)
                outs_pos_th, outs_pos_ccs, outs_pos_ccs_num = remove_size_outliers_ccs(outs_pos_th, min_count=180, max_count=max_count, with_closing=True)
                gts_neg_th, gts_neg_ccs, gts_neg_ccs_num = remove_size_outliers_ccs(gts_neg_th, min_count=180, max_count=max_count, with_closing=True)
                outs_neg_th, outs_neg_ccs, outs_neg_ccs_num = remove_size_outliers_ccs(outs_neg_th, min_count=180, max_count=max_count, with_closing=True)
                ...
                th_to_avoid_overly_diffuse = 3
                cur_precs_per_mass_pos.extend([pos_prec_item for _ in range(min(outs_pos_ccs_num, th_to_avoid_overly_diffuse))])
                cur_recs_per_mass_pos.extend([pos_rec_item for _ in range(min(gts_pos_ccs_num, th_to_avoid_overly_diffuse))])
                cur_precs_per_mass_neg.extend([neg_prec_item for _ in range(min(outs_neg_ccs_num, th_to_avoid_overly_diffuse))])
                cur_recs_per_mass_neg.extend([neg_rec_item for _ in range(min(gts_neg_ccs_num, th_to_avoid_overly_diffuse))])

                # print(f'After pair {i}, average recall per consolidation CC:')
                # if len(cur_recs_per_mass_pos) > 0:
                #     print(f'Positive = {(sum(cur_recs_per_mass_pos) / len(cur_recs_per_mass_pos))}\n')
                # if len(cur_recs_per_mass_neg) > 0:
                #     print(f'Negative = {(sum(cur_recs_per_mass_neg) / len(cur_recs_per_mass_neg))}\n')
                ...
                output_nib = nib.Nifti1Image(gt.cpu().numpy().T, np.eye(4))
                nib.save(output_nib, f'{out_dir}/gt.nii.gz')
                ...
                log_params({'Precision (pos)': pos_prec_item, 'Precision (neg)': neg_prec_item,
                            'Recall (pos)': pos_rec_item, 'Recall (neg)': neg_rec_item,
                            'Dice (pos)': dice_pos, 'Dice (neg)': dice_neg,
                            'CCs in ground truth (pos)': gts_pos_ccs_num, 'CCs in ground truth (neg)': gts_neg_ccs_num,
                            'CCs in output (pos)': outs_pos_ccs_num, 'CCs in output (neg)': outs_neg_ccs_num,
                            'Detected CCs (pos)': pos_rec_item * gts_pos_ccs_num, 'Detected CCs (neg)': neg_rec_item * gts_neg_ccs_num,
                            'Correct output CCs (pos)': pos_prec_item * outs_pos_ccs_num, 'Correct output CCs (neg)': neg_prec_item * outs_neg_ccs_num},
                           f'{out_dir}/params.json')
            ...
            avg_precs_pos.append(avg_prec_pos)
            avg_recs_pos.append(avg_rec_pos)

            avg_dices_neg.append(avg_dice_neg)
            avg_precs_neg.append(avg_prec_neg)
            avg_recs_neg.append(avg_rec_neg)
            ...
            avg_dices_no_FP_no_FN_pos.append(avg_dice_no_FP_no_FN_pos)

            avg_dices_no_FP_neg.append(avg_dice_no_FP_neg)
            avg_dices_no_FN_neg.append(avg_dice_no_FN_neg)
            avg_dices_no_FP_no_FN_neg.append(avg_dice_no_FP_no_FN_neg)
        ...
        idx_dict = {0: "angles (0, 0, 0)", 1: "angles (8, 8, 8)", 2: "angles (15, 15, 15)", 3: "angles (25, 25, 25)"}
